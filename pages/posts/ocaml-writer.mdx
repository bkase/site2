...
In this model, actions are the asynchronous monad. Let's assume we're using Jane Street's [Async](TODO) library and then implemenet `Action` as follows:
...
The concept of knowledge has an interesting property: Once I know something, when presented with new information I'll never[^6] un-know it. As a consequence, knowledge gives rise to a [semi-lattice](TODO link to wikipedia). For `Track_generated.t`:

(insert latex diagram of semi-lattice knowledge)

This means we can implement a well-defined [commutative](TODO)[^7], [associative](TODO)[^8], [idempotent](TODO)[^9] binary operation $$\join$$ that incorporates new knowledge. And in this case, since we don't act when `Cache_hit`, we can treat `Cache_hit` as our identity or bottom state.
...
We can layer it under the async monad[^10] to get a monad that both tracks knowledge and runs code concurrently. Below I'm using monad helpers from Jane Street's [base](TODO) library.
...
[^4]: [Caches are monoids](TODO link to caches monoids video), did you know?

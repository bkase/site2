...
```ocaml
(* OCaml is the only language that can properly express first class
 * implementations anonymously. We do so via the first-class module approach. *)
...
```ocaml
(* OCaml's first-class modules let us upcast without needing to muck with any
 * extra named data types *)
...
```ocaml
(* OCaml supports Rank2 types, but only if they are located inside of data types
 * either inside a record or a GADT *)
